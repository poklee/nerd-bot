...
public class SlackWebhookController
{
    ...
    @PostMapping(path = "/slack", consumes = "application/json", produces = "text/plain")
    public String post(@RequestHeader(name = "X-Slack-Signature", required = false) String slackSignature,
            @RequestHeader(name = "X-Slack-Request-Timestamp", required = false) String slackRequestTimestampString,
            HttpEntity<String> httpEntity)
    {
        ...
        if (validateSignature)
        {
            ...
            {
                // This is a bad signature, that means this request probably did not come from
                // slack
        ...
        {
            LOGGER.warn("Slack signature validation is disabled.  This should only used for local testing");
        }
